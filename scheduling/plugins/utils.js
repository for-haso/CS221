...
G.generativeProcess = function(title) { return blockquote({title: 'Generative process: '.fontcolor('blue')+title, contents: Array.prototype.slice.call(arguments).slice(1)}); }
...
G.factorNode = function(label, options) {
  ...
  if (!label) return c;
  return overlay(c, text(label).orphan(true)).center();
}

G.squareFactor = function() { return square(10).fillColor('white'); }

// Line between a and b with a square factor.
G.edgeFactor = function(a, b) {
  var l = line(a, b);
  return overlay(l.strokeWidth(2), center(squareFactor()).shift(l.xmiddle(), l.ymiddle()));
}
...
G.naiveBayesModel = function(opts) {
  return overlay(
    ytable(
      ...
      opts.pause ? pause() : _,
      xtable(
        x1 = factorNode('$X_1$', opts.condition ? {color: 'gray'} : _),
        x2 = factorNode('$X_2$', opts.condition ? {color: 'gray'} : _),
        '$\\dots$',
        x4 = factorNode('$X_{L}$', opts.condition ? {color: 'gray'} : _),
      _).margin(20).center(),
    _).center().margin(30),
    arrow(y, x1),
    arrow(y, x2),
    ...
    opts.example ? moveTopOf('sports'.fontcolor('red'), y).scale(0.6) : _,
    opts.pause ? pause() : _,
    opts.example ? moveBottomOf('Giants'.fontcolor('red'), x1).scale(0.6) : _,
    opts.example ? moveBottomOf('win'.fontcolor('red'), x2).scale(0.6) : _,
  _);
}
...
G.hmm = function(opts) {
  ...
  if (opts.values) {
    items.push(moveTopOf('(3,1)'.fontcolor('red'), xs[0]).scale(0.6));
    items.push(moveTopOf('(3,2)'.fontcolor('red'), xs[1]).scale(0.6));
  }
  if (opts.pause) items.push(pause());
  if (opts.values) {
    items.push(moveBottomOf('4'.fontcolor('red'), es[0]).scale(0.6));
    items.push(moveBottomOf('5'.fontcolor('red'), es[1]).scale(0.6));
  }
  return overlay(
...
G.chainFactorGraph = function(opts) {
  ...
  for (var i = 0; i < opts.n; i++) {
    var x = factorNode('$X_{'+(i+1)+'}$', opts.xfocus != null && opts.xfocus != (i+1) ? {color: 'gray'} : null);
    nodes.push(x);
  }
  var edges = [];
  for (var i = 0; i < opts.n; i++) {
